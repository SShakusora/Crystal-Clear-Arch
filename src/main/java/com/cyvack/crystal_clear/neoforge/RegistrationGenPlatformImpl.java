...
public class RegistrationGenPlatformImpl {
    ...
    public static <B extends GlassEncasedShaft, P> NonNullUnaryOperator<BlockBuilder<B, P>> glassEncasedshaft(String modId, String casing, Boolean clear) {
        return b -> b
                .blockstate((ctx, prov) -> axisBlock(ctx, prov, state -> prov.models().withExistingParent(ctx.getName(), ResourceLocation.fromNamespaceAndPath(modId, "block/glass_encased_shaft/block"))
                        .texture("casing", ResourceLocation.fromNamespaceAndPath(modId, "block/" + casing + (clear ? "_clear_glass" : "_glass") + "_casing"))
                        .texture("opening", getOpening(modId, casing)), true))
                .item().model((ctx, prov) -> prov.withExistingParent(ctx.getName(), ResourceLocation.fromNamespaceAndPath(modId, "block/glass_encased_shaft/block"))
                        .texture("casing", ResourceLocation.fromNamespaceAndPath(modId, "block/" + casing + (clear ? "_clear_glass" : "_glass") + "_casing"))
        ...
        return b -> b
                .blockstate((ctx, prov) -> axisBlock(ctx, prov, blockState -> {
                    ...
                    return prov.models()
                            ...
                            //Particle
                            .texture("particle", ResourceLocation.fromNamespaceAndPath(modId, "block/" + name + "_casing"))
                            //Casing
                            .texture("casing", ResourceLocation.fromNamespaceAndPath(modId, "block/" + name + "_casing"))
                            //Backing
